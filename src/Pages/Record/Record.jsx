...
const Record = () => {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      if (dataClientsToday === '') {
        clientAPI.getDataALLUsers().then((result) => {
          ...
          console.log(test.concat(dataClientToday, data1800));
    ...
    // const Test_1 = dataClients.filter(arr => arr.info.some(infoDate => infoDate.date.slice(0, 10) === dateToday));
      // setUser1(dataClients.filter(arr => arr.info.some(infoDate => infoDate.date.slice(0, 10) === dateToday)));
      // console.log("user1: ", user1);
    }
    }, [dataClientsToday]);
  // console.log(dataClientsToday)
